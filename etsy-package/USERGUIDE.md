...
the **Mini Mission Control** listing on Etsy.
...
├── MissionControl-v*/            📦 Build staging dir (rebuilt on demand)
├── MissionControl-v*.zip         📦 Final ZIP (this is what the buyer downloads)
│
...
- **What:** Node script that renders the 6 printable HTML templates to
  ...
  everything into `MissionControl-v<version>.zip`.
...
- **Outputs:**
  ...
  - `etsy-package/MissionControl-v<version>/` (staging dir, rebuilt)
  - `etsy-package/MissionControl-v<version>.zip` (the buyer ZIP)
...
- **What:** "We Link Two Devices in 3 Seconds!" poster in 5 big kid-safe steps.
...
- **The seven mockups:**
  ...
  2. **9-cell feature grid** — 6 panels × features
...
11. Attach the digital file: `MissionControl-v<version>.zip`.
...
17. Keep old ZIPs tracked in Git (the `.gitignore` exemption keeps
    `etsy-package/MissionControl-v*.zip` in history) so you can always
