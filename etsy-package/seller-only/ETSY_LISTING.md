# Etsy listing copy — Mini Mission Control
...
Bluetooth Web App for two devices (laptop + phone) + Illustrated Guide | 8 Tabs, 5 Robots, Live Graphs | STEM Classroom | Digital Download

> Product name in your Etsy shop is **"Mini Mission Control"** — use the phrase **"for the two devices (laptop + phone)"** everywhere else to stay within the Micro:bit Educational Foundation's trademark-use guidelines.
...
> **Browser-based Bluetooth control panel for the two devices (laptop + phone). No install. Works on Chromebook. Chrome or Edge required — not iPhone / iPad / Safari.**
>
> ✅ **Tested on Chromebook** — primary target device for schools. Installs as a PWA, works offline after the first load.
>
> 🎮 **Mini Mission Control** turns your micro:bit into an interactive STEM lab — no installs, no accounts, no cloud. Just open it in your browser and connect.
>
...
>
> ⚙️ **Requires**:
> • two devices (laptop + phone) (sold separately)
...
Because this is an instant download, I can't offer refunds once the ZIP has been delivered — please double-check compatibility (Chrome or Edge, desktop, two devices (laptop + phone)) before buying. If the product doesn't work as described on a supported browser, message me within 14 days and I'll make it right.
...
>
> A printable A3 **two devices (laptop + phone) pin-out poster** — the exact diagram used in the Mini Mission Control illustrated guide. Labeled pins, accelerometer axes, sound sensor callout, touch-logo arrow, radio + BLE note.
>
...
>
> 💬 Loved it? Check out the full **Mini Mission Control** app in my shop — the browser Bluetooth control panel this poster was designed for.
...
> Hi [First Name] — thank you for grabbing Mini Mission Control with the launch code! 🎉
>
...
| #2 | Français | Translate EN title, keep "for the two devices (laptop + phone)" English | From `etsy-playbook-fr.html` | Same footage, FR end-card text |
...
```
Hey {{BUYER_FIRST_NAME}} — thanks so much for grabbing Mini Mission Control! 🎮
...
>
> 📘 **Start here:** unzip the file and open **SETUP.md** (5-minute guide). You'll need a two devices (laptop + phone) and Chrome or Edge — Safari and iPhones won't work (that's a Web Bluetooth limitation, not the app).
>
...
>
> 1. Do you have a **two devices (laptop + phone)** (the one with the gold microphone)? V1 is missing the sound sensor and a couple of features.
...
> Hi! Just checking in — were you able to get the Mini Mission Control running this week? If you ran into anything, reply here and I'll sort it out. If it worked smoothly, a star rating on the listing helps me keep building tools like this. Either way, thanks for the support!
...
| 0:12 – 0:15 | End card: logo + text overlay "Mini Mission Control — works in Chrome / Edge" | Hold for 3 seconds, this is the loop seam |
...
> *(0:00)* "This is the Mini Mission Control. It runs in your browser — no installs."
>
...
A weekly 5-minute "what you can build with Mini Mission Control" video, ending with a soft *"if you want this app, link's in the description"*. Channel takes 6 months to compound but pays forever. Skip if you don't enjoy being on camera.
...
**Why:** One site-license sale = 15 individual sales. Same files, different LICENSE. Zero extra work once the ZIP is built. Buyer file: `~/Desktop/micromission-control-v1.0-site-license.zip` (already built).

### Title

```
School Site License for the two devices (laptop + phone) Bluetooth Web App | Up to 30 Teachers, Unlimited Students | STEM Classroom | Instant Digital Download
```
...
> • The full Mini Mission Control web app (runs locally in any Chromebook / laptop)
> • Ready-to-flash firmware (makecode.ts) for two devices (laptop + phone)
...
>
> ⚙️ **Requires:** two devices (laptop + phone) (sold separately) + Chrome or Edge. Not supported: Safari, Firefox, iPhone, iPad (Web Bluetooth limitation).
>
...
> Thank you for licensing Mini Mission Control for your school! 🏫
>
...
**Files:** same ZIP as the single-user listing (`micromission-control-v1.0.zip`). The UI auto-switches to Arabic on first launch if the browser locale is `ar-*`, or the user picks it from the language dropdown.
...
```
🎮 لوحة تحكم مايكروبت — تطبيق متكامل عبر البلوتوث للوحة two devices (laptop + phone)
...
⚙️ يتطلب:
• لوحة two devices (laptop + phone) (تُشترى بشكل منفصل)
...
📘 ابدأ من هنا: فك ضغط الملف وافتح SETUP.md (دليل 5 دقائق). تحتاج لوحة two devices (laptop + phone) ومتصفح Chrome أو Edge — لا يعمل على Safari أو iPhone (قيود Web Bluetooth).
...
• 1× 4 GB USB stick containing the complete Mini Mission Control app
• 1× Pre-flashed two devices (laptop + phone) (firmware already loaded, shows the X icon on first power-on)
...
| two devices (laptop + phone) (wholesale from PiHut/ElecFreaks) | $12.00 |
...
#### 3a. Listing title
```
Mini Mission Control — District-Wide STEM Licensing | Quote for 3+ Schools | Includes Teacher Training Materials
```
...
|-------|-------|-------------|
| 1 | **Cover** | Logo + "Mini Mission Control — District-Wide Licensing Proposal for [District Name]" + your name + date |
...
| 3 | **What it is** | Screenshot of the app + tab map. One sentence: "A browser-based Bluetooth control panel for two devices (laptop + phone) — no install, no account, no cloud." |
...
My name is [Your Name], and I am the developer of Mini Mission Control — a browser-based STEM tool already in use by educators in [countries]. I am writing to propose a District-Wide License for [District Name].
...
2. Stripe Dashboard → **Products** → add `Mini Mission Control v9` (placeholder for next release).
...
Last year you bought Mini Mission Control on Etsy (order #[X]). Thank you! I just shipped v9 — it has [new feature 1], [new feature 2], and [new feature 3].
...
- 4 certificate styles: beginner ("I made my first LED"), intermediate ("I built a sensor dashboard"), advanced ("I built a robot"), mastery ("Mini Mission Control Certified")
...
Description:
🏆 Reward your students — 4 printable certificates for Mini Mission Control milestones.
• I Made My First LED
• Sensor Scientist
• Robot Builder
• Playground Master
...
Perfect as end-of-workshop rewards, homeschool milestones, or parent-facing proof of progress. Works standalone — no app purchase required, but designed to pair with our Mini Mission Control.
...
Want to plot the temperature, record sessions, and export CSV data without writing any extra code? My browser-based **Mini Mission Control** does all of that out of the box (it's what I used to take the screenshots in this tutorial). You can find it here: [Etsy URL].
...
I'm the developer of Mini Mission Control — a browser-based app that turns a bare micro:bit V2 into an interactive STEM lab (LEDs, sensors, servos, graphs, 3D). You can see it at [Etsy URL].
